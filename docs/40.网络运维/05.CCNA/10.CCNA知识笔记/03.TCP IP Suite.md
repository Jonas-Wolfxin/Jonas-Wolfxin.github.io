...
>
> - **IHL(Internet Header Length) 报头长度**：4 bits, 标明IPv4的Header的大小，**其单位是32bit即4 Bytes**，其最小值为5（`5 x 4 = 20 bytes`），从上图中看出，其规定头部长为 4 bit，所以最大值为 15， `15 x 4 = 60 byte`. 还可以算出可选项长度最大为40个字节(即 `60 byte - 20 byte = 40 byte`)
>
